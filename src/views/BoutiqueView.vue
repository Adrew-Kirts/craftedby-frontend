    ...
    <div class="main-content grid grid-cols-1 md:grid-cols-5 md:gap-5 px-5 md:px-24">
      ...
      <div class="block md:hidden">
        <div class="flex flex-row mt-10 ml-8 relative">
          <button @click="toggleFiltersDropdown" aria-label="Ouvrir les filtres">
            <img alt="Filter Icon" src="../assets/icons/filters.svg">
          </button>
          <p class="ml-2">FILTRER</p>
        </div>
        <div v-show="showFilters" class="filters-dropdown z-20">
          <CategoryFilterComponent @categoryChanged="fetchProductsByCategory" aria-label="Filtrer par catégorie" />
          <ColorFilterComponent @colorsChanged="fetchProductsByColor" aria-label="Filtrer par couleur" />
          <PriceFilterComponent @priceChanged="fetchProductsByPrice" aria-label="Filtrer par prix" />
      ...
      <div
        ...
        <div v-for="product in products" :key="product.id" class="result card max-w-72">
          ...
          <div>
            ...
            <CTAButtonBase v-if="product.stock > 0" text="AJOUTER AU PANIER" aria-label="Ajouter {{ product.name }} au panier" @click="addToCart(product)" />
            <CTAButtonBase v-else-if="product.stock <= 0" text="PLUS DISPONIBLE" aria-label="Produit {{ product.name }} non disponible" />
          </div>
        </div>

        <notification-component aria-live="polite" :type="notificationType" :message="notificationMessage" />
